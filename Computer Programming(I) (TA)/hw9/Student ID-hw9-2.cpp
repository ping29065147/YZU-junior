...
void readDictionary( string *dictionary, int &numDicWords )
{
    ...
    while (inFile >> tmp) dictionary[numDicWords++] = tmp;
}
   ...
   for( int i = 0; i < numWords; i++ )
      for( size_t j = 0; j < length; j++ )
      {
          if (words[i][j] != wordToCheck[j])
          {
              ...
              buffer2 = wordToCheck;
              buffer1[j] = buffer2[j] = ' ';
              if (buffer1 == buffer2) result[numResult++] = words[i];
              break;
          }
      }
...
void spellCheck2( string wordToCheck, string *dictionary, int numDicWords, string *result, int &numResult )
{
   ...
   for( int i = 0; i < numWords; i++ )
      for( size_t j = 0; j < length; j++ )
      {
          ...
          if (words[i][j] != wordToCheck[j])
          {
              for (int k = 0; k < j; ++k) buffer += words[i][k];
              for (int k = j + 1; k < length; ++k) buffer += words[i][k];
              if (buffer == wordToCheck) result[numResult++] = words[i];
              break;
          }
      }
...
void spellCheck3( string wordToCheck, string *dictionary, int numDicWords, string *result, int &numResult )
{
   ...
   for( int i = 0; i < numWords; i++ )
      for( size_t j = 0; j < wordToCheck.size(); j++ )
      {
          ...
          if (words[i][j] != wordToCheck[j])
          {
              for (int k = 0; k < j; ++k) buffer += wordToCheck[k];
              for (int k = j + 1; k <= length; ++k) buffer += wordToCheck[k];
              if (buffer == words[i]) result[numResult++] = words[i];
              break;
          }
      }

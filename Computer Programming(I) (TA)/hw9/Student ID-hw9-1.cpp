...
void readDictionary( char ( *dictionary )[ 21 ], int &numDicWords )
{
    ...
    while (inFile >> tmp) strcpy_s(dictionary[numDicWords++], tmp);
}
                  ...
                  int numDicWords, char ( *&result )[ 21 ], int &numResult )
{
   ...
   for (int i = 0; i < numWords; i++) 
      ...
      {
          if (words[i][j] != wordToCheck[j])
          {
              strcpy_s(buffer1, words[i]);
              strcpy_s(buffer2, wordToCheck);
              buffer1[j] = buffer2[j] = ' ';
              if (strcmp(buffer1, buffer2) == 0) strcpy_s(result[numResult++], words[i]);
              break;
          }
      }
                  ...
                  int numDicWords, char ( *&result )[ 21 ], int &numResult )
{
   ...
   for (int i = 0; i < numWords; i++)
       for (int j = 0; j < length; j++)
       {
           if (words[i][j] != wordToCheck[j])
           {
               for (int k = 0; k < j; ++k) buffer[k] = words[i][k];
               for (int k = j + 1; k < length; ++k) buffer[k - 1] = words[i][k];
               buffer[length - 1] = '\0';
               if (strcmp(buffer, wordToCheck) == 0) strcpy_s(result[numResult++], words[i]);
               break;
           }
       }
                  ...
                  int numDicWords, char ( *&result )[ 21 ], int &numResult )
{
   ...
   for( int i = 0; i < numWords; i++ )
      for( unsigned int j = 0; j < strlen( wordToCheck ); j++ )
      {
          if (words[i][j] != wordToCheck[j])
          {
              for (int k = 0; k < j; ++k) buffer[k] = wordToCheck[k];
              for (int k = j + 1; k <= length; ++k) buffer[k - 1] = wordToCheck[k];      
              buffer[length] = '\0';
              if (strcmp(buffer, words[i]) == 0) strcpy_s(result[numResult++], words[i]);
              break;
          }
      }

...
void multiplication( const vector< Term > &multiplicand, const vector< Term > &multiplier, vector< Term > &product )
{
   ...
   if( multiplicand.size() != 0 && multiplier.size() != 0 )
   {
       ...
       for (int i = 0; i < multiplier.size(); ++i)
       {
           ...
           {
               buffer[j].expon = multiplicand[j].expon + multiplier[i].expon;
               buffer[j].coef = multiplicand[j].coef * multiplier[i].coef;
           }
               ...
               vector< Term > &quotient, vector< Term > &remainder )
{
   ...
   while (remainder.size() != 0 && remainder[0].expon >= divisor[0].expon)
   {
       ...
       monomial[0].expon = temp[quotient.size()].expon;
       monomial[0].coef = temp[quotient.size()].coef;
   ...
   for (int i = 0; i < quotient.size(); ++i) assign(quotient[i], temp[i]);
   //----------

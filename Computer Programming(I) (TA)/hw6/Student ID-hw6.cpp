...
void addition( int *addend, int *adder, int &addendDegree, int adderDegree )
{
    addendDegree = std::max(addendDegree, adderDegree);
    for (int i = 0; i <= adderDegree; ++i) addend[i] += adder[i];
    while (!addend[addendDegree] && addendDegree) --addendDegree;
}

// minuend -= subtrahend
void subtraction( int *minuend, int *subtrahend, int &minuendDegree, int subtrahendDegree )
{
    minuendDegree = std::max(minuendDegree, subtrahendDegree);
    for (int i = 0; i <= subtrahendDegree; ++i) minuend[i] -= subtrahend[i];
    while (!minuend[minuendDegree] && minuendDegree) --minuendDegree;
    if( minuendDegree != 0 && minuend[ minuendDegree ] == 0 ) cout << "Leading zeroes not allowed!\n";
}
...
void division( int *dividend, int *divisor, int *quotient, int *remainder,
               int dividendDegree, int divisorDegree, int quotientDegree, int &remainderDegree )
{
    for (int i = 0; i <= (remainderDegree = dividendDegree); ++i) remainder[i] = dividend[i];
    for (int tmp = quotientDegree; tmp >= 0; --tmp)
    {
        quotient[tmp] = remainder[remainderDegree] / divisor[divisorDegree];

        int monomialDegree = tmp;
        int* monomial = new int[monomialDegree + 1]();
        monomial[monomialDegree] = quotient[monomialDegree];

        int bufferDegree = divisorDegree + monomialDegree;
        int* buffer = new int[bufferDegree + 1]();
